        ...
        public static async Task EventAsync(CommandsNextExtension cnt, CommandErrorEventArgs e)
        {
            ...
            switch (e.Exception)
            {
                ...
                case UnauthorizedException ux:
                    e.Context.Client.Logger.LogDebug(new EventId(601, "Comando Invalido"), $"[{e.Context.User.Username}({e.Context.User.Id})] tentou usar '{e.Command?.QualifiedName ?? "<comando desconhecido>"}' mas deu erro: {e.Exception}\ninner:{e.Exception?.InnerException}.", DateTime.Now);
                    break;

                case ServerErrorException se:
                    e.Context.Client.Logger.LogInformation(new EventId(601, "Comando Invalido"), $"[{e.Context.User.Username}({e.Context.User.Id})] tentou usar '{e.Command?.QualifiedName ?? "<comando desconhecido>"}' mas deu erro: {e.Exception}\ninner:{e.Exception?.InnerException}.", DateTime.Now);
                    break;

                case Newtonsoft.Json.JsonReaderException _:
                    e.Context.Client.Logger.LogInformation(new EventId(602, "Discord Problem"), $"[{e.Context.User.Username}({e.Context.User.Id})] tentou usar '{e.Command?.QualifiedName ?? "<comando desconhecido>"}' mas discord está lento.", DateTime.Now);
                    break;
                ...
                case ArgumentException ae:
                    if (ae.Message == "Could not find a suitable overload for the command.")
                    {
                        ...
                        e.Context.Client.Logger.LogDebug(new EventId(601, "Argument Error"), $"[{e.Context.User.Username}({e.Context.User.Id})] tentou usar '{e.Command?.QualifiedName ?? "<comando desconhecido>"}' mas deu erro: {e.Exception}\ninner:{e.Exception?.InnerException}.", DateTime.Now);
                    break;

                default:
                    e.Context.Client.Logger.LogDebug(new EventId(601, "Command Error"), $"[{e.Context.User.Username}({e.Context.User.Id})] tentou usar '{e.Command?.QualifiedName ?? "<comando desconhecido>"}' mas deu erro: {e.Exception}\ninner:{e.Exception?.InnerException}.", DateTime.Now);
                    break;
            }
        }
    }
